...
describe("LoaNFT", function () {
  ...
  it("Should be able to receive a loan request with an NFT as collateral", async function () {
    ...
    expect(ownerOfFirstBefore).not.to.equal(
      ethers.constants.AddressZero,
      "The owner of the first minted token is the address zero"
    );
    expect(ownerOfFirstBefore).to.equal(
      owner.address,
      "The owner of the first NFT is not the sender"
    );
    ...
    const { amount, tokenId, applicant, yearlyInterestRate, loanDuration } =
      await loaNFT.getLoanRequest(requestId);

    expect(amount).to.eq(expectedAmount, "Amount does not match");
    expect(tokenId.toNumber()).to.equal(
      expectedTokenId,
      "TokenId does not match"
    );
    expect(applicant.toString()).to.equal(
      owner.address,
      "Applicant does not match"
    );
    expect(yearlyInterestRate).to.equal(
      expectedYearlyInterestRate,
      "Yearly interest rate does not match"
    );
    expect(loanDuration).to.equal(
      expectedLoanDuration,
  ...
  it("Should be possible to receive the loan when the liquidity has been provided", async function () {
    ...
    expect(balanceBefore).to.be.lte(
      balanceAfter,
      "You didn't got the money for the loan"
    );
  ...
  it("Should not be possible to receive a loan twice", async function () {
    ...
    expect(loaNFT.widthrawLoan(requestId)).to.be.revertedWith(
  ...
  it("Should not be possible to withdraw a non existent loan", async function () {
    ...
    const wrongRequestId = ethers.utils.solidityKeccak256(
      ["address", "address", "uint256"],
  ...
  it("Should not be possible to get the money back twice", async function () {
    ...
    expect(
      loaNFT.connect(supplier).redeemLoanOrNFT(requestId)
    ).to.be.revertedWith("The loan does not exist");
  });
  ...
  it("Should be possible to get the NFT back if the loan has not been repaid within the deadline", async function () {
    ...
    expect(nftOwnerBefore).to.be.equal(
      loaNFT.address,
      "NFT was not owned by the contract"
    );
    expect(nftOwnerAfter).to.be.equal(
      supplier.address,
      "NFT has not been sent to the supplier"
    );
  });
  ...
  it("Should not be possible to get back an NFT from a loan that does not belong to you", async function () {
    ...
    expect(
      loaNFT.connect(thirdParty).redeemLoanOrNFT(requestId)
    ).to.be.revertedWith("This loan has not been funded by you");
  });

  it("Should be possible to handle multiple loans at the same time", async function () {
    const [firstOwner, firstSupplier, secondOwner, secondSupplier] =
    ...
    const secondTokenIdBignumber = await testNFT
      .connect(secondOwner)
      .getLastId();
    ...
    const firstRequestId = ethers.utils.solidityKeccak256(
      ["address", "address", "uint256"],
      [firstOwner.address, testNFT.address, expectedTokenId]
    );

    const secondRequestId = ethers.utils.solidityKeccak256(
      ["address", "address", "uint256"],
      [secondOwner.address, testNFT.address, secondTokenId]
    );

    await loaNFT
      .connect(secondSupplier)
      .provideLiquidityForALoan(secondRequestId, {
        value: expectedAmount.toString(),
      });

    await loaNFT
      .connect(firstSupplier)
      .provideLiquidityForALoan(firstRequestId, {
        value: expectedAmount.toString(),
      });

    let firstBalanceBefore = await ethers.provider.getBalance(
      firstOwner.address
    );

    await loaNFT.connect(firstOwner).widthrawLoan(firstRequestId);

    let firstBalanceAfter = await ethers.provider.getBalance(
      firstOwner.address
    );

    expect(firstBalanceBefore).to.be.lt(
      firstBalanceAfter,
      "Balance has not increased"
    );

    await ethers.provider.send("evm_increaseTime", [
      expectedLoanDuration - 1000000,
    ]);
    await ethers.provider.send("evm_mine", []);

    let secondBalanceBefore = await ethers.provider.getBalance(
      secondOwner.address
    );

    await loaNFT.connect(secondOwner).widthrawLoan(secondRequestId);

    let secondBalanceAfter = await ethers.provider.getBalance(
      secondOwner.address
    );

    expect(secondBalanceBefore).to.be.lt(
      secondBalanceAfter,
      "Balance has not increased"
    );
    ...
    const secondInterests: BigNumber = await loaNFT.getLoanInterests(
      secondRequestId
    );
    ...
    expect(secondBalanceBefore).to.be.gt(
      secondBalanceAfter,
    ...
    const firstInterests: BigNumber = await loaNFT.getLoanInterests(
      firstRequestId
    );
    ...
    expect(firstBalanceBefore).to.be.gt(
      firstBalanceAfter,
      "Loan has not been repaid"
    );

    const contractBalanceAtTheEnd = await ethers.provider.getBalance(
      loaNFT.address
    );
