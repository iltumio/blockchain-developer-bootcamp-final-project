...
contract LoaNFT is Ownable, IERC721Receiver, Interest {
   ...
   */
  function getLoanRequest(bytes32 _requestId) public view returns (LoanRequest memory) {
   ...
   */
  function _addLoanRequest(bytes32 requestId, LoanRequest memory _loanRequest) internal {
  ...
  function _removeRequest(bytes32 requestId) internal {
    ...
    if (index != lastIndex) {
      ...
      bytes32 lastRequestId = _computeRequestId(last.applicant, last.erc721contract, last.tokenId);
      // Update the Index
  ...
  function _removeLoan(bytes32 _loanId) internal {
    ...
    if (index != lastIndex) {
      ...
      bytes32 lastLoanId = _computeRequestId(last.applicant, last.erc721contract, last.tokenId);
      // Update the Index
  ...
  function requestLoan(
    ...
    bytes32 requestId = _computeRequestId(msg.sender, erc721contract, tokenId);
    require(
      loanRequestsTracker[requestId] == 0 || loanRequestsTracker[requestId] == MAX_UINT,
  ...
  function widthrawLoan(bytes32 loanId) public {
    require(loansTracker[loanId] != 0 && loansTracker[loanId] != MAX_UINT, 'The loan does not exist');

    Loan memory loan = getLoan(loanId);

    require(loan.applicant == msg.sender, 'This loan does not belong to you');
    require(loan.status == LoanStatus.FUNDED, 'The loan is in the wrong state');
  ...
  function getLoanInterests(bytes32 loanId) public view returns (uint256) {
    ...
    uint256 effectiveRatePerSecond = yearlyRateToRay(loan.yearlyInterestRate);
  ...
  function repayLoan(bytes32 loanId) public payable {
    ...
    require(loan.applicant == msg.sender, 'This loan does not belong to you');
    require(loan.status == LoanStatus.ACTIVE, 'The loan is not active. Nothing to repay');
  ...
  function redeemLoanOrNFT(bytes32 loanId) public payable {
    ...
    require(loan.supplier == msg.sender, 'This loan has not been funded by you');
    require(
      loan.status == LoanStatus.REPAID || (loan.status == LoanStatus.ACTIVE && block.timestamp > loan.deadline),
      "Conditions to redeem the loan doesn't match"
    );

    if (loan.status == LoanStatus.REPAID) {
      _payLoanBack(loanId);
    } else if (loan.status == LoanStatus.ACTIVE && block.timestamp >= loan.deadline) {
  ...
  function _sendNFTToTheSupplier(bytes32 loanId) internal {
    ...
    IERC721(loan.erc721contract).safeTransferFrom(address(this), msg.sender, loan.tokenId);
  }
